...
A`Calendar` can be used inline on a page and is also used by the `useDateInput` and `useDateRangeInput` hooks.  
  
Alternatively you could use to compose your own date picker components.
...
| clickOutsideWhiteList| `array`| optional| `[]`| Array of refs to ignore clicks, when determining whether the user clicked outside the Calendar|
