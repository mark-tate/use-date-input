...
const createYearSource = (fromDate, { addYears, startOfMonth, subtractYears, toFormattedDate }) => {
    const yearStart = subtractYears(startOfMonth(fromDate), 5);
    return [...Array(10)].map((unused, index) => {
        const year = toFormattedDate(addYears(yearStart, index), formatNames.HEADER);
        return {
            key: `year-${year}`,
            value: year,
            label: year
...
export default function AdditionalControlHeader() {
    const classes = useStyles();
    const { visibleFromDate } = useCalendarState();
    const dateAPI = useDateAPI();
    const { navigateNext, navigatePrevious, setVisibleFromDate } = useCalendarDispatch();
    ...
    const handleMonthChange = month => {
        const newVisibleFromDate = dateAPI.set(visibleFromDate, { month: month - 1 });
        setVisibleFromDate(newVisibleFromDate);
    };
    ...
    return (
        <div className={classes.root}>
            ...
            <Dropdown
                ...
                selectedValue={dateAPI.toFormattedDate(visibleFromDate, formatNames.MONTH)}
                source={monthSource}
            />
            <Dropdown
                label={'Year'}
                ...
                selectedValue={dateAPI.toFormattedDate(visibleFromDate, formatNames.YEAR)}
                source={yearSource}
            />
